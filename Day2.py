...
# Ensure similarity.pkl file is fetched from Git LFS before using it
def fetch_lfs_file(file_path):
    if not os.path.exists(file_path):
        st.error(f"The file {file_path} is not available. Please make sure it's fetched using Git LFS.")
        return None
...
def load_data():
    ...
    # Ensure the similarity.pkl file is properly fetched from Git LFS
    similarity = fetch_lfs_file('similarity.pkl')
...
if movies is None or similarity is None:
    st.error("Failed to load movie data or similarity matrix. Please ensure the files are available.")
else:
    # Streamlit UI
